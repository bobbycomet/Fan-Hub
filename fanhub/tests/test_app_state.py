...
class TestDaemonControllerMock(unittest.TestCase):
    """
    DaemonController logic — mocks subprocess and init detection.
    Tests status parsing, SIGHUP delivery, and per-init dispatch.
    """
    def setUp(self):
        import core.init_system as _is
        _is._CACHED = 'systemd'     # force systemd path for most tests
        from core.daemon_controller import DaemonController
        self.DC = DaemonController

    def tearDown(self):
        import core.init_system as _is
        _is._CACHED = None          # reset so other tests get real detection

    def test_status_active_enabled(self):
        import unittest.mock as mock
        responses = {
            ('systemctl', 'is-active',  'fanhub-daemon'): (0, 'active'),
            ('systemctl', 'is-enabled', 'fanhub-daemon'): (0, 'enabled'),
        }
        def fake_run(*args, **kw):
            return responses.get(args, (1, ''))
        with mock.patch.object(self.DC, '_run', side_effect=fake_run):
            st = self.DC.status()
        self.assertTrue(st.active)
        self.assertTrue(st.enabled)
        self.assertTrue(st.installed)
        self.assertFalse(st.no_init_support)

    def test_status_not_installed(self):
        import unittest.mock as mock
        responses = {
            ('systemctl', 'is-active',  'fanhub-daemon'): (3, 'inactive'),
            ('systemctl', 'is-enabled', 'fanhub-daemon'): (1, 'not-found'),
        }
        def fake_run(*args, **kw):
            return responses.get(args, (1, ''))
        with mock.patch.object(self.DC, '_run', side_effect=fake_run):
    ...
    def test_status_unknown_init(self):
        import core.init_system as _is, unittest.mock as mock
        _is._CACHED = 'unknown'
        st = self.DC.status()
        self.assertTrue(st.no_init_support)
        self.assertFalse(st.installed)

    def test_summary_running_enabled(self):
        from core.daemon_controller import DaemonStatus
        st = DaemonStatus(installed=True, active=True, enabled=True, init='systemd')
    ...
    def test_summary_not_installed(self):
        from core.daemon_controller import DaemonStatus
        st = DaemonStatus(installed=False, active=False, enabled=False, init='systemd')
        text, color = st.summary()
        self.assertIn('not installed', text.lower())
        self.assertNotEqual(color, '#44ff88')

    def test_summary_unsupported_init(self):
        from core.daemon_controller import DaemonStatus
        st = DaemonStatus(False, False, False, 'unknown', no_init_support=True)
        text, color = st.summary()
        self.assertIn('not directly supported', text.lower())

    def test_reload_sends_sighup_via_process(self):
        import unittest.mock as mock, os
        with mock.patch.object(self.DC, 'is_active', return_value=True), \
             mock.patch.object(self.DC, '_find_daemon_pid', return_value=12345), \
             mock.patch('os.kill') as mock_kill:
            result = self.DC.reload()
        self.assertTrue(result)
        mock_kill.assert_called_once_with(12345, 1)  # signal.SIGHUP == 1
    ...
    def test_runit_status_reads_sv(self):
        import core.init_system as _is, unittest.mock as mock, os
        _is._CACHED = 'runit'
        with mock.patch('os.path.isdir', return_value=True), \
             mock.patch('os.path.islink', return_value=True), \
             mock.patch.object(self.DC, '_sv', return_value=(0, 'run: fanhub-daemon: (pid 999) 5s')):
            st = self.DC.status()
        self.assertTrue(st.installed)
        self.assertTrue(st.enabled)
        self.assertTrue(st.active)
        self.assertEqual(st.init, 'runit')

    def test_openrc_status_reads_rc_service(self):
        import core.init_system as _is, unittest.mock as mock, os
        _is._CACHED = 'openrc'
        with mock.patch('os.path.isfile', return_value=True), \
             mock.patch.object(self.DC, '_rc_service', return_value=(0, 'started')), \
             mock.patch.object(self.DC, '_rc_update', return_value=(0, 'fanhub-daemon')):
            st = self.DC.status()
        self.assertTrue(st.installed)
        self.assertTrue(st.active)
        self.assertEqual(st.init, 'openrc')
