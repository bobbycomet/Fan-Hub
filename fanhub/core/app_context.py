...
class AppContext:
    ...
    # Fan override — routes through PollingWorker.set_fan_manual/auto
    # so GUI, daemon, and curve engine all stay in sync via IPC
    set_fan_manual:       Callable[[str, float], None] = field(default=lambda fid, spd: None)
    set_fan_auto:         Callable[[str], None] = field(default=lambda fid: None)
    set_all_auto:         Callable[[], None] = field(default=lambda: None)
    toggle_pwm_inverted:  Callable[[str, bool], None] = field(default=lambda fid, inv: None)
