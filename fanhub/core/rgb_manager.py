...
def find_openrgb_binary() -> Optional[str]:
    ...
    Returns None if not found — caller should direct user to
    https://openrgb.org or the Codeberg AppImage download.
    """
    # 1. PATH
    ...
    # 3. RPM / non-Debian package managers
    for rpm_cmd in [['rpm', '-ql', 'openrgb'], ['pacman', '-Ql', 'openrgb']]:
        try:
            r = subprocess.run(rpm_cmd, capture_output=True, text=True, timeout=3)
            if r.returncode == 0:
                for line in r.stdout.strip().split('\n'):
                    line = line.strip()
                    if line and os.path.isfile(line) and os.access(line, os.X_OK):
                        logger.info(f"OpenRGB via {rpm_cmd[0]}: {line}")
                        return line
        except Exception:
            pass

    # 4. AppImage — search common locations (any version name)
    appimage_dirs = [
    ...
    for d in appimage_dirs:
        ...
        for entry in sorted(os.listdir(d)):
            if 'openrgb' in entry.lower() and not entry.lower().endswith('.deb'):
                full = os.path.join(d, entry)
                if os.path.isfile(full) and os.access(full, os.X_OK):
                    logger.info(f"OpenRGB AppImage found: {full}")
                    return full
                if os.path.isfile(full) and entry.lower().endswith('.appimage'):
                    # Not +x — chmod and return
                    try:
                        os.chmod(full, 0o755)
                        logger.info(f"OpenRGB AppImage chmod +x: {full}")
                        return full
                    except Exception:
                        logger.info(f"OpenRGB AppImage found (not +x, chmod failed): {full}")
