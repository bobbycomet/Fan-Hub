...
from core.fan_curves import FanCurve, CurvePoint, PRESET_CURVES
...
class CurveEditorCanvas(QWidget):
    ...
    def set_current_temp(self, temp: float):
        ...
        if self.points:
            curve = FanCurve(name='_', points=self.points)
            self._current_speed = curve.interpolate(temp)
        self.update()
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._dragging >= 0:
            ...
            self.points.sort(key=lambda p: p.temp)
            # BUG FIX: after sort the dragged index may have shifted; re-find it
            self._dragging = self._find_nearby_point(x, y)
            if self._dragging < 0:
                # point moved out of grab radius — keep closest
                self._dragging = min(
                    range(len(self.points)),
                    key=lambda i: abs(self.points[i].temp - temp)
                )
            self.points_changed.emit(self.points)
            self.update()
        else:
...
class FanCurvesTab(QWidget):
    ...
    def _build_ui(self):
        ...
        sl.addWidget(QLabel("Sensor (driver):"))
