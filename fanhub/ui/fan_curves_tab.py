...
from PyQt6.QtWidgets import (
    ...
    QCheckBox, QSpinBox, QLineEdit, QMessageBox, QFrame, QScrollArea
)
...
class FanCurvesTab(QWidget):
    ...
    def _build_ui(self):
        ...
        outer.setContentsMargins(12, 4, 12, 10)
        outer.setSpacing(0)

        # Splitter lets the user resize the left panel, and (unlike fixed-width
        # group boxes) it degrades gracefully on narrow windows — the panel
        # shrinks to its minimum width and the scroll area below handles any
        # vertical overflow, instead of clipping content off-screen.
        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setChildrenCollapsible(False)
        outer.addWidget(splitter)

        # ── Left panel — wrapped in a scroll area ─────────────────────────────
        ...
        curve_group.setMinimumWidth(200)
        ...
        settings_group.setMinimumWidth(200)
        ...
        # Scroll area so tall content (all three group boxes) never gets
        # clipped when the window is short — a vertical scrollbar appears
        # instead of controls disappearing off the bottom of the screen.
        ...
        # Canvas gets priority when resizing; left panel keeps its width
        # unless the user explicitly drags the splitter handle.
