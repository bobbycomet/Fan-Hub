...
from PyQt6.QtWidgets import (
    ...
    QDialog, QDialogButtonBox, QCheckBox, QFrame, QFileDialog
)
...
class ProfilesTab(QWidget):
    ...
    def __init__(self, profile_manager, curve_engine, rgb_manager, state,
        ...
        self.state = state
        self._ctx = ctx  # AppContext — may be None for legacy callers
        self._build_ui()
        self._refresh_list()
    ...
    def _build_ui(self):
        ...
        exp_btn.setToolTip("Export selected profile to a .json file for sharing")
        exp_btn.clicked.connect(self._export_profile)
        io_row.addWidget(exp_btn)
        imp_btn = QPushButton("⬇ Import")
        imp_btn.setToolTip("Import a profile from a .json file")
        imp_btn.clicked.connect(self._import_profile)
    ...
    def _refresh_tray(self):
        ...
        if self._ctx is not None:
            try:
                self._ctx.on_tray_menu_refresh()
                return
            except Exception:
                pass
        # Fallback: legacy parent-walk (ctx not provided)
        try:
            w = self.parent()
            while w is not None:
    ...
    def _export_profile(self):
        """Export selected profile to a .json file."""
        row = self.profile_list.currentRow()
        if row < 0:
            QMessageBox.information(self, "Export", "Select a profile to export first.")
            return
        item = self.profile_list.item(row)
        name = item.text().lstrip("✓ ")
        if name.startswith("("):
        ...
        profile = self.state.get_profile(name)
        if not profile:
            QMessageBox.warning(self, "Export", f"Profile '{name}' not found.")
            return
        path, _ = QFileDialog.getSaveFileName(
            self, f"Export profile '{name}'",
            f"{name.replace(' ', '_')}.json",
            "Fan Hub Profile (*.json)")
        if not path:
            return
        try:
    ...
    def _import_profile(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Fan Hub Profile", "",
            "Fan Hub Profile (*.json);;All files (*)")
        if not path:
            return
        try:
            ...
            if not name:
                ...
                profile['name'] = name
            # Ask if overwrite when name already exists
    ...
    def _apply_quick_preset(self, preset_name: str):
        ...
        # Get fan list from context, fall back to parent-walk
        hw = None
        if self._ctx is not None:
            hw = self._ctx.hw_monitor
        if hw is None:
            w = self.parent()
            while w is not None:
    ...
    def _sync_daemon(self):
        """Persist curves to config and signal the daemon."""
        if self._ctx is not None:
            try:
                self._ctx.on_curves_changed()
                return
            except Exception:
                pass
        # Fallback: legacy parent-walk
        try:
            w = self.parent()
            while w is not None:
                if hasattr(w, '_save_curves_to_config'):
                    w._save_curves_to_config()
                    return
                w = w.parent()
        except Exception:
            pass
