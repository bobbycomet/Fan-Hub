...
class FirstRunWizard(QDialog):
    ...
    def _build_ui(self):
        ...
        # Page 1: system config (only shown when not yet configured)
        self._stack.addWidget(self._page_system_configure())
    ...
    def _page_system_configure(self) -> QWidget:
        """Page 1: run install.sh via pkexec."""
        from ui.appimage_configure_dialog import AppImageConfigureDialog, _detect_distro, _PM_NAMES, _INIT_NAMES
        ...
        h1 = QLabel("Configure Fan Hub for Your System")
        ...
        detected = QLabel(
            f"Detected: <b>{_dpretty}</b>  |  "
            f"Package manager: <b>{_pm_label}</b>  |  "
            f"Init system: <b>{_init_label}</b>"
        )
        detected.setTextFormat(Qt.TextFormat.RichText)
        detected.setStyleSheet("color:#88bbcc; font-size:12px;")
        detected.setWordWrap(True)
        l.addWidget(detected)

        desc = QLabel(
            "Fan Hub needs to install system components to control your fans:\n"
            "  • udev rules — targeted PWM access for your user\n"
            "  • fanhub group — group-based permission model\n"
            "  • kernel modules — nct6775, it87, coretemp\n"
            "  • daemon service — keeps curves running at boot\n\n"
            "Click Configure below, then authenticate with your password. "
        ...
        self._wizard_cfg_status = QLabel("")
        self._wizard_cfg_status.setWordWrap(True)
        self._wizard_cfg_status.setStyleSheet("font-size:12px;")
        l.addWidget(self._wizard_cfg_status)

        self._wizard_cfg_btn = QPushButton("⚙  Configure Now (requires password)")
        self._wizard_cfg_btn.setObjectName("applyBtn")
        self._wizard_cfg_btn.clicked.connect(self._run_wizard_configure)
        l.addWidget(self._wizard_cfg_btn)

        skip_lbl = QLabel(
            "<small style='color:#445566'>You can skip this and configure later "
            "from Settings → Configure System, but fan control may not work until then.</small>"
        )
        skip_lbl.setTextFormat(Qt.TextFormat.RichText)
        skip_lbl.setWordWrap(True)
        l.addWidget(skip_lbl)
        l.addStretch()
        return w

    def _run_wizard_configure(self):
        ...
        def on_configured():
            self._system_configured = True
            self.state.settings['system_configure_done'] = True
            self.state.save_config()
            self._wizard_cfg_status.setText(
                "✓ System configured successfully. Click Next to continue.")
            self._wizard_cfg_status.setStyleSheet("color:#44ff88; font-size:12px;")
            self._wizard_cfg_btn.setText("✓ Done — Click Next to continue")
            self._wizard_cfg_btn.setEnabled(False)
        dlg.configured.connect(on_configured)
        dlg.exec()
    ...
    def _page_welcome(self) -> QWidget:
        ...
        if not self._system_configured:
            sysconf_banner = QLabel(
                "⚠  Fan Hub has not yet been configured for your system.\n"
                "Step 1 will set up udev rules, groups, kernel modules and the "
                "daemon for your distro. Click Next to begin."
            )
            sysconf_banner.setWordWrap(True)
            sysconf_banner.setStyleSheet(
                "color:#ffcc44; font-size:12px; background:#1a1000; "
                "border:1px solid #6a4400; border-radius:4px; padding:10px 12px;")
            l.addWidget(sysconf_banner)

        body = QLabel(
            "This setup wizard will take you through the steps to get Fan Hub working:\n\n"
            + ("1.  Configure Fan Hub for your system (udev rules, groups, daemon)\n"
               "2.  Scan your hardware and show which fans can be controlled\n"
               "3.  Confirm what Fan Hub found on your system\n"
               "4.  Apply a safe default fan curve so your fans are managed immediately\n"
               if not self._system_configured else
    ...
    def _go_to(self, index: int):
        # Skip system configure page (index 1) when already configured
        if index == 1 and self._system_configured:
            index = 2  # jump to fans page
        self._stack.setCurrentIndex(index)
        total = 5 if not self._system_configured else 4
        self._prog.setRange(0, total)
        self._prog.setValue(index + 1)
        if self._system_configured:
            titles = ["Welcome", "—", "Hardware Scan", "Choose Fan Curve", "Setup Complete"]
        else:
            titles = ["Welcome", "Configure System", "Hardware Scan",
                      "Choose Fan Curve", "Setup Complete"]
        self._title_label.setText(titles[min(index, len(titles)-1)])
        display_step = index if self._system_configured else index + 1
        self._step_label.setText(f"Step {display_step} of {total}")
        self._back_btn.setEnabled(index > 0)
        done_idx = 4
        self._skip_btn.setVisible(index < done_idx)
        if index == done_idx:
    ...
    def _go_next(self):
        ...
        # Page 1 = system configure: mark done when advancing past it
        if idx == 1 and not self._system_configured:
            self._system_configured = True
            self.state.settings['system_configure_done'] = True
            self.state.save_config()
        if idx == 3:  # preset page — apply before done
            self._apply_preset()
            ctrl = sum(1 for f in self.hw.fans.values()
                       if f.controllable and f.pwm_file)
