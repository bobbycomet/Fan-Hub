...
class LiquidTab(QWidget):
    ...
    def late_init(self, manager):
        ...
        self.manager = manager
        self._panels.clear()
        # Clear the existing layout WITHOUT replacing it (calling setLayout twice
        # on an existing widget causes a Qt warning and leaks the old layout).
        ...
        if existing is not None:
            while existing.count():
                ...
                if w is not None:
                    w.setParent(None)
                    w.deleteLater()
            # Populate the cleared layout directly instead of calling _build_ui
            # which would try to create a new QVBoxLayout
            self._populate_layout(existing)
        else:
            self._build_ui()
    ...
    def _populate_layout(self, layout):
        """Fill a pre-existing layout (used by both _build_ui and late_init)."""
