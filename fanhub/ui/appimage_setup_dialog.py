...
class AppImageSetupDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # Daemon checkbox
        # Detect init system and show appropriate daemon note
        from core.init_system import detect_init, init_name as _iname
        _init = detect_init()
        _daemon_hint = {
            'systemd': 'systemd (systemctl)',
            'runit':   'runit (sv) — enable with: sudo ln -s /etc/sv/fanhub-daemon /var/service/',
            'openrc':  'OpenRC (rc-service) — enable with: sudo rc-update add fanhub-daemon default',
        }.get(_init, 'your init system')
        self._daemon_cb = QCheckBox(
            f"Also install the background daemon ({_daemon_hint})\n"
            "(keeps fan curves running at boot and when the app is closed)")
        self._daemon_cb.setChecked(_init in ('systemd', 'runit', 'openrc'))
