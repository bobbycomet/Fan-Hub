    ...
    def _build_ui(self, zero_rpm_fans: list, all_fans: dict):
        ...
        if zero_rpm_fans:
            ...
            for fan in zero_rpm_fans:
                ...
                row.addWidget(dot)
                conn_type = fan.connection_type if hasattr(fan, 'connection_type') else ''
                ctrl = 'PWM' if getattr(fan, 'pwm_file', None) else 'DC/No-control'
                info = QLabel(
                    f"<b>{fan.label}</b> — {fan.chip_name} "
                    f"(type: {conn_type}, {ctrl})"
                )
        ...
        # NVIDIA-specific note if any NVIDIA fans are in the 0-RPM list
        nvidia_fans = [f for f in zero_rpm_fans
                       if getattr(f, 'gpu_vendor', '') == 'nvidia']
        if nvidia_fans:
            nv_box = QGroupBox("ℹ  NVIDIA GPU Fans — Expected Behaviour")
            nv_box.setStyleSheet(
                "QGroupBox { border:1px solid #1a6644; color:#44dd88; "
                "margin-top:22px; padding-top:12px; } "
                "QGroupBox::title { top:4px; left:10px; }")
            nl = QVBoxLayout(nv_box)
            nv_lbl = QLabel(
                "<b>NVIDIA GPU fans show 0 RPM — this is normal on Linux.</b><br><br>"
                "NVIDIA GPUs on Linux do not expose a hardware tachometer (RPM sensor) "
                "via the standard hwmon interface unless CoolBits is enabled. "
                "Fan Hub controls the fan using a <b>percentage</b> (0–100%), "
                "not RPM, and the percentage shown is accurate.<br><br>"
                "<b>To verify your GPU fan is working:</b><br>"
                "→ Go to <b>Fan Control</b> tab and set the GPU fan to 80%.<br>"
                "→ Listen for the fan to spin up. If it does, everything is working correctly.<br>"
                "→ NVIDIA fans only show RPM if <b>CoolBits=4</b> is set in your Xorg config, "
                "or <b>nvidia-settings</b> is installed and running."
            )
            nv_lbl.setWordWrap(True)
            nv_lbl.setStyleSheet("color:#88bbaa; font-size:12px;")
            nl.addWidget(nv_lbl)
            cl.addWidget(nv_box)
