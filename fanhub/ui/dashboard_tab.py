...
class TempGauge(QFrame):
    ...
    def __init__(self, label: str, warning: float = 80.0, critical: float = 90.0):
        ...
        # Split "Source — Sensor Name" into two lines for readability
        # e.g. "Motherboard — VRM Temperature" →
        #        line1: "Motherboard"   line2: "VRM Temperature"
        line1, line2 = self._split_label(label)

        if line2:
            # Source prefix on top in muted colour
            src_lbl = QLabel(line1)
            src_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            src_lbl.setStyleSheet(
                "color: #445566; font-size: 9px; background: transparent;")
            src_lbl.setWordWrap(False)
            layout.addWidget(src_lbl)

            self.name_lbl = QLabel(line2)
        else:
            self.name_lbl = QLabel(line1)

        self.name_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.name_lbl.setObjectName("gaugeLabel")
        self.name_lbl.setToolTip(label)
        self.name_lbl.setWordWrap(True)
        self.name_lbl.setStyleSheet("font-size: 10px; background: transparent;")
    ...
    def _split_label(self, label: str):
        """
        Split a label into (prefix, detail) at ' — ' or ' - ' separators.
        Returns (full_label, '') if no separator found.
        """
        for sep in (' — ', ' - ', ': '):
            if sep in label:
...
class FanCard(QFrame):
    ...
    def update_data(self, data):
        ...
        Uses getattr() with fallbacks so this also tolerates being passed
        a dict directly (e.g. from tests or future refactors).
        """
        if isinstance(data, dict):
            ...
            hub  = getattr(data, 'is_hub_channel', False)
...
class TempHistoryChart(QWidget):
    ...
    def update_temps(self, temps: dict, hw_temps: dict = None):
        """
        temps:     {sid: current_value}
        hw_temps:  hw_monitor.temps dict for label lookup (optional)
        """
        self.tick += 1
        ...
        for sid, value in temps.items():
            if sid not in self.history:
                ...
                if CHARTS_AVAILABLE:
                    ...
                    # Use the human-readable sensor label when available
                    if hw_temps and sid in hw_temps:
                        full_label = hw_temps[sid].label
                        # Shorten: strip prefix up to ' — ' for the legend
                        if ' — ' in full_label:
                            legend_label = full_label.split(' — ', 1)[1]
                        elif ': ' in full_label:
                            legend_label = full_label.split(': ', 1)[1]
                        else:
                            legend_label = full_label
                    else:
                        legend_label = sid.split('_temp')[0].replace('hwmon', 'hw')

                    series.setName(legend_label[:24])
...
class DashboardTab(QWidget):
    ...
    def showEvent(self, event):
        """Force layout recalculation every time the tab becomes visible."""
        super().showEvent(event)
        # Qt doesn't always re-run heightForWidth on re-show.
        # Explicitly invalidate and recalculate so FlowLayout wraps correctly.
        if hasattr(self, 'temp_container'):
            self.temp_flow.invalidate()
            self.temp_container.updateGeometry()
            self.temp_container.adjustSize()
        if hasattr(self, 'fan_container'):
            self.fan_flow.invalidate()
            self.fan_container.updateGeometry()
    ...
    def _build_ui(self):
        ...
        # ── System Overview strip (CPU/GPU/RAM/Storage/Network) ───────────────
        from ui.system_overview_widget import SystemOverviewWidget
        self.overview = SystemOverviewWidget()
    ...
    def _init_temp_gauges(self):
        # Remove all existing gauge widgets from the layout before rebuilding
        while self.temp_flow.count():
            item = self.temp_flow.takeAt(0)
            if item and item.widget():
                item.widget().deleteLater()
        self._temp_gauges.clear()
    ...
    def _init_fan_cards(self):
        # Remove all existing fan cards from the layout before rebuilding
        while self.fan_flow.count():
            item = self.fan_flow.takeAt(0)
            if item and item.widget():
                item.widget().deleteLater()
        self._fan_cards.clear()
    ...
    def update_temps(self, temps: dict):
        ...
        self.chart.update_temps(temps, hw_temps=self.hw.temps)

    def update_fans(self, fans: dict):
        for fid, data in fans.items():
            if fid not in self._fan_cards:
                # data is a FanEntry object — read .label directly, not .get()
                label = (data.get('label', fid) if isinstance(data, dict)
                         else getattr(data, 'label', fid))
    ...
    def update_liquid(self, devices: list):
        ...
        if not devices:
            # Collapse to a slim single-line hint rather than an empty
            # full-height section — reduces visual clutter when the user
            # has no AIO/liquid cooling hardware at all.
            self.liquid_group.setTitle("Liquid Cooling (none detected)")
            self.liquid_group.setMaximumHeight(34)
            lbl = QLabel("Connect an AIO or USB fan/liquid hub to see it here.")
            lbl.setStyleSheet("color: #334455; font-size: 11px;")
            self.liquid_layout.addWidget(lbl)
            self.liquid_layout.addStretch()
            return

        self.liquid_group.setTitle("Liquid Cooling")
        self.liquid_group.setMaximumHeight(16777215)   # Qt's default max (unset the cap)
