...
class MainWindow(QMainWindow):
    ...
    def _setup_tray(self):
        ...
        _tray_icon = QSystemTrayIcon(self._make_tray_icon(), app)
        _tray_icon.setToolTip("Fan Hub — running in background")
        ...
        _tray_icon.activated.connect(self._tray_activated)
    ...
    def enable_tray(self, enabled: bool):
        global _tray_icon
        if enabled and _tray_icon is None:
            self._setup_tray()
        elif not enabled and _tray_icon is not None:
    ...
    def _on_sensors_updated(self, temps: dict):
        ...
        if temps:
            ...
            et = self.state.settings.get('emergency_temp', 90.0)
            et_display = et * 9/5 + 32 if unit == 'F' else et
            self.emergency_label.setText(
                f"EMERGENCY: {mt:.0f}{unit_sym}" if mt >= et_display else "")
    ...
    def _show_fan_warning(self):
        fans      = self.hw_monitor.fans
        zero_fans = [f for f in fans.values() if f.current_rpm == 0]
    ...
    def closeEvent(self, event):
        ...
        global _tray_icon
        tray_alive = (_tray_icon is not None and _tray_icon.isVisible())
