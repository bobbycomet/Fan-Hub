...
# Create dedicated group for hwmon access (more secure than world-writable)
groupadd -f fanhub 2>/dev/null || true
usermod -aG fanhub "$ACTUAL_USER" 2>/dev/null || true

cat > "$UDEV_RULES" << 'UDEV'
# Fan Hub — hwmon write access (targeted: only pwm and enable files, not all of hwmon)
# Grants write access to the specific files Fan Hub needs, not the entire hwmon tree.
# Uses the 'fanhub' group so only fanhub group members can write (not world-writable).
...
# Fallback for distros where the above kernel match doesn't fire on hwmon sub-attrs:
# also grant read on all hwmon so temp sensors are readable without root
KERNEL=="hwmon[0-9]*", SUBSYSTEM=="hwmon", ACTION=="add",     RUN+="/bin/sh -c 'chown root:fanhub /sys%p/pwm* 2>/dev/null; chmod 660 /sys%p/pwm* 2>/dev/null; chmod 444 /sys%p/temp*_input 2>/dev/null || true'"
# i2c access
...
# fanhub group created in udev section above
...
# The daemon script is already copied as part of the application files above.
...
# Write the systemd service file directly (do not rely on assets copy)
SERVICE_FILE="/etc/systemd/system/fanhub-daemon.service"
cat > "$SERVICE_FILE" << 'SVCEOF'
[Unit]
Description=Fan Hub headless fan curve daemon
Documentation=man:fanhub-daemon(1)
...
systemctl daemon-reload
# Do NOT auto-enable — user opts in via Settings > Background Daemon
# If updating and daemon was previously enabled, preserve that state
if systemctl is-enabled --quiet fanhub-daemon 2>/dev/null; then
    systemctl restart fanhub-daemon 2>/dev/null &&         ok "Daemon restarted (was already enabled)" ||         warn "Daemon restart failed — check: systemctl status fanhub-daemon"
else
    ok "Daemon service installed. Enable in Fan Hub Settings > Background Daemon."
    log "  Or manually: sudo systemctl enable --now fanhub-daemon"
fi
# ── Summary ───────────────────────────────────────────────────────────────────
echo ""
if [ "$IS_UPDATE" = true ]; then
